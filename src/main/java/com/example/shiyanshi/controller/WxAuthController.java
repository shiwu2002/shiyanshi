...
public class WxAuthController {
    ...
    public Result<Map<String, Object>> loginByCode(@RequestBody Map<String, String> body) {
        try {
            ...
            // 配置RestTemplate以支持将text/plain响应解析为JSON
            RestTemplate client = new RestTemplate();
            
            // 添加String转换器，支持text/plain
            StringHttpMessageConverter stringConverter = new StringHttpMessageConverter(StandardCharsets.UTF_8);
            stringConverter.setWriteAcceptCharset(false);
            
            // 配置JSON转换器，支持text/plain类型
            MappingJackson2HttpMessageConverter jsonConverter = new MappingJackson2HttpMessageConverter();
            List<MediaType> supportedMediaTypes = new ArrayList<>();
            supportedMediaTypes.add(MediaType.APPLICATION_JSON);
            supportedMediaTypes.add(MediaType.TEXT_PLAIN); // 关键：支持text/plain
            supportedMediaTypes.add(new MediaType("application", "*+json"));
            jsonConverter.setSupportedMediaTypes(supportedMediaTypes);
            
            // 设置转换器
            client.getMessageConverters().clear();
            client.getMessageConverters().add(stringConverter);
            client.getMessageConverters().add(jsonConverter);
